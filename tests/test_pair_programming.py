...
class TestReviewFeedback:
    ...
    def test_minimal_feedback(self):
        ...
        feedback = ReviewFeedback(
            chunk_id="chunk_001", feedback_type=FeedbackType.APPROVE, comments=["Looks good"]
        )
    ...
    def test_feedback_to_dict(self):
        ...
        feedback = ReviewFeedback(
            chunk_id="chunk_001", feedback_type=FeedbackType.MINOR, comments=["Minor issue"]
        )
    ...
    def test_has_blocking_issues_major(self):
        ...
        feedback = ReviewFeedback(
            chunk_id="chunk_001", feedback_type=FeedbackType.MAJOR, comments=["Major issue"]
        )
    ...
    def test_has_blocking_issues_blocking(self):
        ...
        feedback = ReviewFeedback(
            chunk_id="chunk_001", feedback_type=FeedbackType.BLOCKING, comments=["Critical issue"]
        )
        assert feedback.has_blocking_issues()
    ...
    def test_has_blocking_issues_minor(self):
        ...
        feedback = ReviewFeedback(
            chunk_id="chunk_001", feedback_type=FeedbackType.MINOR, comments=["Small issue"]
        )
    ...
    def test_has_blocking_issues_approve(self):
        ...
        feedback = ReviewFeedback(
            chunk_id="chunk_001", feedback_type=FeedbackType.APPROVE, comments=["LGTM"]
        )
        assert not feedback.has_blocking_issues()
...
class TestPairExchange:
    ...
    def test_full_exchange(self):
        ...
        feedback = ReviewFeedback(
            chunk_id="chunk_001", feedback_type=FeedbackType.APPROVE, comments=["Good"]
        )
        revision = CodeChunk(
        ...
        exchange = PairExchange(
            exchange_id=1, chunk=chunk, feedback=feedback, revision=revision, resolved=True
        )
...
class TestPairConfig:
    ...
    def test_custom_config(self):
        ...
        config = PairConfig(
            max_revisions_per_chunk=5, timeout_seconds=300, verbose=False, dev_model="sonnet"
        )
...
class TestPairSession:
    ...
    def mock_dependencies(self):
        ...
        with (
            patch("lib.pair_programming.get_shared_memory") as mock_sm,
...
"""
    ...
    def test_build_dev_prompt_initial(self, mock_dependencies):
        ...
        prompt = session._build_dev_prompt(
            task_part="Implement login", context="Auth module context", previous_feedback=None
        )
    ...
    def test_build_dev_prompt_with_feedback(self, mock_dependencies):
        ...
        prompt = session._build_dev_prompt(
            task_part="Continue implementation", context="Context", previous_feedback=feedback
        )
    ...
    def test_build_reviewer_prompt(self, mock_dependencies):
        ...
        prompt = session._build_reviewer_prompt(chunk=chunk, accumulated_code="previous code here")
        assert "PAIR PROGRAMMING" in prompt
